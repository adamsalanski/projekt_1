...
class Transformacje:
    ...
    def XYZ2neu(self,s,alfa,z,X,Y,Z):
        ...
        w układzie topocentrycznym. Aby dokonać tej transformacji należy wczesniej wyznaczyć wektor przestrzenny s,alfa,z czyli wektor
        ze srodkiem w miejscu obserwacji - początku układu NEU. 
    ...
    def zargparse(self):
            ...
            parser.add_argument('-metoda', help='metoda', choices=['fl2pl1992', 'BL22000', 'hirvonen', 'flh2XYZ', 'XYZ2neu'], required=False, type = str)
            ...
            if args.X:
                ...
                Y=float(args.Y)
                Z=float(args.Z)
                if args.metoda == 'hirvonen':
                    f,l,h = trans.hirvonen(X,Y,Z)
                elif args.metoda == 'XYZ2neu':
                    neu = trans.XYZ2neu(X,Y,Z,s,alfa,z)
            elif args.f:
                f=float(args.f)
                l=float(args.l)
                if not args.H:
                    args.H = 0
                h=float(args.H)
                if args.metoda == 'flh2XYZ':
                    X,Y,Z = trans.flh2XYZ(f,l,h)
                elif args.metoda == 'BL22000':
                    if args.ns:
                        ns=5
                    elif args.ns:
                        ns=6
                        ...
                        ns=8
                    x2000,y2000= trans.BL22000(f,l,ns)
                elif args.metoda == 'fl2pl1992':
                    x1992,y1992 = trans.fl2pl1992(f, l)
            
            
        
            
            return( args.X, args.Y, args.Z, args.f, args.l, args.H, args.s, args.alfa, args.z, args.ns, args.metoda)
...
if __name__ == "__main__":
    ...
    print('WYNIKI WYWOŁANIA Z KONSOLI')
    proba = Transformacje()
    X,Y,Z,f,l,h,s,alfa,z,ns, metoda = proba.zargparse()
    if metoda == 'hirvonen':
        print('(f,l,h)',f,l,h)
    elif metoda == 'flh2XYZ':
        print('(X,Y,Z)',X,Y,Z)
    elif metoda == 'fl2pl1992':
